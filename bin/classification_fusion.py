...
from src.models.calm_classifier import CalmClassifier, ModelArguments
...
def load_model(model_args: ModelArguments) -> Any:
    if model_args.calm_model_name_or_path is not None:
        return CalmClassifier(model_args)
    config = tf.AutoConfig.from_pretrained(model_args.text_model_name_or_path)
    return tf.AutoModelForSequenceClassification.from_pretrained(
        model_args.text_model_name_or_path,
        from_tf=bool(".ckpt" in model_args.text_model_name_or_path),
        config=config,
    )
...
def run(
    ...
    tokenizer = tf.AutoTokenizer.from_pretrained(model_args.text_model_name_or_path)
    ...
    if not data_args.do_regression:
        model.label2id={v: i for i, v in enumerate(label_list)}
        model.id2label={i: v for i, v in enumerate(label_list)}
